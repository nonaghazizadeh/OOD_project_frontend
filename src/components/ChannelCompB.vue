<template>
  <div>
    ...
    <div v-else dir="rtl" class="channel-page container-fluid">
      <div class="row page">
        ...
        <div class="col-2 sidebar no-float">
          ...
          <div
            v-if="channelSearchMode"
            class="list-group mt-3 w-100 channel-list"
          >
            <div v-for="channel in searchedChannelList" :key="channel.id">
              <a
                class="list-group-item list-group-item-action flex-column align-items-start"
                :class="{ active: activeTag === channel.id }"
                @click="selectChannel(channel.id, channel)"
              >
                <div class="d-flex w-100 channel-list-name">
                  <img
                    src="../assets/images/channelimg.jpeg"
        ...
        <div
          v-else-if="activeTag !== null && !contentLoading"
          class="col content no-float"
        >
          <div class="row top-content">
            ...
            <div v-show="!currentChannel.isJoin" class="col-1">
              <font-awesome-icon
                ...
                class="channel-info-icon"
                @click="joinChannel(currentChannel.joinLink)"
              />
            </div>
            ...
            <div class="col-1">
              <router-link
                :to="{ name: 'infochannel', query: { id: currentChannel.id } }"
                class="info-channel-router"
              >
                <font-awesome-icon
          ...
          <div
            :class="[isUser ? 'row center-content-user' : 'row center-content']"
          >
            <div class="col-8">
              <b-spinner
                class="channel-loader"
                ...
                <div class="card mt-3">
                  <div class="card-body">
                    <div class="mb-3">
                      ...
                      <span class="edit-icon" v-show="!isUser">
                        <font-awesome-icon
                          ...
                          @click="goToEditContent()"
                        />
                      </span>
                      <span
                        class="trash-icon"
                        v-show="!isUser"
                        @click="removeContent(item.contentId)"
                      >
                    ...
                    <p v-if="isUser" class="card-text">
                      <a
                        :href="`http://79.127.54.112:5000/${item.fileName}`"
                        target="_blank"
                      >
                        {{ item.fileName }}
                      </a>
            ...
            <div
              v-else-if="!contentSearchMode && !contentLoading"
              class="col-8"
            >
    ...
    >
      <template #default>
        <p class="modal-text">
          برای مشاهده کامل محتوا می‌توانید هزینه آن را پرداخت کنید و یا با رفتن
          به بخش اطلاعات کانال حق‌عضویت خریداری کنید. آیا می‌خواهید این محتوا را
          بخرید؟ (هزینه محتوا ۱۰۰۰ تومان است)
        </p>
      </template>
    ...
    goToAddContent() {
      this.$router.push({
        ...
        query: {
          ...
          name: this.channels[this.channelIndex].name,
        },
      });
    },
    goToEditContent() {
